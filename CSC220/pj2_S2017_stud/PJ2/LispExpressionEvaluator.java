...
public class LispExpressionEvaluator {
    ...
    private void evaluateCurrentOperation() {
        ...
        String operation = null;
         if (inputExprStack.size()==1) {
            throw new LispExpressionEvaluatorException("Empty input or wrong input.");
        }
        do {
        ...
        if (evaluationStack.size() == 0) {
            throw new LispExpressionEvaluatorException("No number.");
        }   
        switch (operation) {
            case "+":
            ...
            case "/":
                result = evaluationStack.pop();
                if (result == 0) {
                        throw new LispExpressionEvaluatorException("Can not divide.");
                    }
                if (evaluationStack.size() == 0) {
                    result = 1.0 / result;
                }
    ...
    public double evaluate() {
        ...
        //to trake the totaled number of input operators and operations
        //to trake the position of ")"
        // Step 1: Scan the tokens in the string.
        while (inputExprScanner.hasNext()) {
            ...
            if (inputExprScanner.hasNextInt()) {
                ...
                //inputExprStack.push((double)inputExprScanner.nextInt());
                ...
                switch (item) {
                    ...
                    case '(':
                        char nextToken = inputExprScanner.next().charAt(0);
                        // Step 4: If you see an operator, push operator object onto the inputExprStack
                        if (nextToken == '+' || nextToken == '-'
                            ...
                            throw new LispExpressionEvaluatorException("If you input '(', next input shoube an operator");
                        }
                    // Step 5: If you see ")"  // steps in evaluateCurrentOperation() :
                    case ')':
        ...
        double result = 0.0;
        if (inputExprStack.size() != 1) {
            throw new LispExpressionEvaluatorException("Wrong input.");
        } else {
           
            try {
                result = Double.parseDouble((inputExprStack.pop()).toString());
            } catch (Exception e) {
                throw new LispExpressionEvaluatorException("Result is not double.");
            }
        }
return result;
    }
